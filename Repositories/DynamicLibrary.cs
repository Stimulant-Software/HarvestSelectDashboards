...
#pragma warning restore 0219
        ...
        Expression ParseStringLiteral() {
            ...
            var singleQuoteCount = token.text.Count(item => item == quote);
            if (singleQuoteCount == 2) {
                var lastIndex = token.text.LastIndexOf(quote);
                if (lastIndex == token.text.Length - 1) {
                    string s = token.text.Substring(1, token.text.Length - 2);
                    NextToken();
                    return CreateLiteral(s, s);
                }
            } else {
                var nextPos = token.text.IndexOf(token.text, 1, quote);
                string s = token.text.Substring(1, nextPos -1);
                int start = 0;
                while (true) {
